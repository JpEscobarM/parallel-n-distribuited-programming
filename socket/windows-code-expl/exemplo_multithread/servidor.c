...
void *funcao_thread (void * args){
        ...
        printf("fechando o socket do cliente...\n");
        closesocket(sock_cli);
        printf("socket do cliente fechado...\n\n");
}
/*----------------------------------------------------------------------------*/
...
int main(){
    ...
    while ( 1 ) {
        ...
        CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) funcao_thread, (void *) sock_cli, 0, (LPDWORD) &threadID);



    }

    printf("fechando o socket do server...\n");
    closesocket(sock_srv);
    printf("socket do server fechado...\n\n");
